...
export class CodepenController {
  ...
  static startCodepen() {
    ...
    let enabled =
      $globalCodepenConfig.length &&
      $globalCodepenConfig.attr("enabled") &&
      $globalCodepenConfig.attr("enabled").match(/^(1|y|yes|t|true)$/i);

    const $codepenConfig = $(
      ".remark-visible .remark-slide-content codepen:not([global])"
    );
    ...
    if ($codepenConfig.length) {
      ...
      disabled =
        $codepenConfig.attr("disabled") !== undefined &&
        $codepenConfig.attr("disabled") !== false;
    ...
    $(
      ".remark-visible .remark-slide-content code.javascript:not(.codepen)"
    ).each(function (i) {
  ...
  start() {
    ...
    this.sourceId = `codepen-comem-archioweb-${sha1(this.source)}-${
      this.index
    }`;
    ...
    this.$element.attr(
      "data-tippy-content",
      "Click on CodePen to run this code"
    );
    this.tip = tippy(
      ".remark-visible .remark-slide-content .codepen[data-tippy-content]"
    );
  }
